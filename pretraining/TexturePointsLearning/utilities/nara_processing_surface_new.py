...
if __name__ =="__main__":
    ...
    print('pts3d: ', pts3d.shape, "pts_uv: ", pts_uv.shape) ## (100000, 3), ,(100000, 2)
    print(np.max(pts3d[:,0]),np.min(pts3d[:,0])) ## 0.7883843925489555 -0.5527986884418117
    print(np.max(pts3d[:,1]),np.min(pts3d[:,1])) ## 0.07194496434598784 -0.3398539713640302
    print(np.max(pts3d[:,2]),np.min(pts3d[:,2])) ## 1.7112682269142525 0.001905703769033082
    print("normal noise shape: ", normal_noise.shape, np.min(normal_noise), np.max(normal_noise) )
