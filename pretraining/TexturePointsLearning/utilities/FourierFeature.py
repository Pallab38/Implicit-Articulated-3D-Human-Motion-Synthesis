...
def input_mapping(x: torch.Tensor, scale: float = 10.0) -> torch.Tensor:
    # x = np.random.randn(1, 1024, 1024, 3)
...
if __name__ == "__main__":
    ...
    print(f"enc_x: \nShape:{enc_x.shape}, \ndtype:{enc_x.dtype}")
    print(
        f"enc_x: {torch.min(enc_x).item(), torch.max(enc_x).item(), torch.sum(enc_x).item()}"
    )
    gamma_x = input_mapping_torch(x)
    print(
        f"From torch: {gamma_x.shape, torch.min(gamma_x).item(), torch.max(gamma_x).item(), torch.sum(gamma_x).item() }"
    )
