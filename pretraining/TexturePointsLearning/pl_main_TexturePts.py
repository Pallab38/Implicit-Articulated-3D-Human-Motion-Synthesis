...
class PainterDensityNet(torch.nn.Module):
    ...
    def forward(
        self, painter_in: torch.Tensor, density_in: torch.Tensor
...
class LitPainterDensity(pl.LightningModule):
    def __init__(
        self,
        my_painterDensity: torch.nn.Module,
        person: int,
        mesh_num: int,
        n_sample: int,
        save_path: str,
        ...
        self.my_texPts = TexturePoints(
            self.person_list, mesh_num, n_sample, self.init_epoch, save_path, my_device
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        (
            ...
            self.points_texture,
            optim_tex,
        ) = self.my_texPts.get_texPts_for_training(self.person_list)
        ...
        painter_input = torch.cat(
            (gamma_uvw, latent_smpl, fft_normDirection), dim=2
        ).to(
            torch.float32
        )  ## [1, 300000, 1096]
        ...
        pulsar_out = pulsar(
            pts_3d, pulsar_in.clone(), vert_radii, rvec, Cs, Ks, opacity=out_dense
        )
        ...
        optim_dict = (
            optim.__dict__
        )  ## ['defaults', '_zero_grad_profile_name', 'state', 'param_groups', '_warned_capturable_if_run_uncaptured', '_optimizer', '_strategy', '_optimizer_idx', '_on_before_step', '_on_after_step']
        # optim_state = optim_dict["state"]  ## defaultdict(<class 'dict'>, {})
        optim_defaults = optim_dict[
            "defaults"
        ]  ## ['lr', 'betas', 'eps', 'weight_decay', 'amsgrad', 'maximize', 'foreach', 'capturable']
        # print(f'defaults: {optim_defaults.keys(), optim_defaults}')
        lr = optim_defaults["lr"]

        epoch = trainer.current_epoch
        out_root = (
            f"/home/user/output/LearnTexturePoints/{self.person_list[0]}_{my_lt_dim}Ch"
        )
        ...
        l1_loss = nn.L1Loss()
        loss_pulsarImg = l1_loss(
            pred_image, gt_image
        )  ###  (-0.08053641021251678, 1.0), (0.0,1.0)
    ...
    def test_step(self, batch, batch_idx):
        ...
        (
            ...
            self.points_texture,
            optim_tex,
        ) = self.my_texPts.get_texPts_for_training(self.pid)
        gamma_uvw = input_mapping_torch(self.points_texture.clone(), self.device)
        # print(f"gamma_uvw: {gamma_uvw.shape, gamma_uvw.is_cuda, gamma_uvw.requires_grad}")
        latent = batch["latent"]  ## [bs1, w512, h512, c3]
        latent = rearrange(
...
if __name__ == "__main__":
    ...
    try:
        ...
        ################################################
        model_path = join(
            pl_log_dir, "version_NoDir/checkpoints", "epoch=49-step=1050.ckpt"
        )  ##E107, pred_img[-1,1], opacity=density
