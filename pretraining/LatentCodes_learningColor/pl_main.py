...
def init_processes(rank, size, fn, my_args, my_queue,backend="gloo"):
    ...
    out = fn(my_args["pts"], my_args["lt_img"], my_args["rad"],
            my_args["r"], my_args["cs"], my_args["ks"])
...
class LitUNet(pl.LightningModule):
    def __init__(self,train_path,latent_dim,
                 person, bs=1,
        ...
        self.latent_codes = LatentCodes(self.latent_dim, self.train_path, [self.person],
                                        -1, self.device,
    ...
    def training_step(self,batch,batch_idx):
        ...
        pts3d, pts_uv, normal_noise = batch["pts3d"].float(), batch["pts_uv"].float(),batch["normal_noise"] ## [1, 300000, 3]  [1, 300000, 2]
        pid, gt_img, gt_mask = batch["pid"], batch["image"], batch["mask"] ## "313",(1, 3, 512, 512),(1, 1, 512, 512)
        self.width, self.height = gt_img.shape[2],gt_img.shape[2] ## width: 1024, height:1024
        rvec, Cs, Ks = batch["rvec"], batch["Cs"], batch["Ks"] #### rvec: torch.Size([1, 3]), Cs: torch.Size([1, 3]), Ks: torch.Size([1, 3, 3])
        vert_radii,normals = batch["vert_radii"], batch["normals"].detach().cpu().numpy() ## ###(n_pts x 1)  (batch_size x n_pts x 3) 
        ...
        self.latent, optims = self.latent_codes.get_codes_for_training(list(pid)) ###[bs=1,w=512,h=512,ch=3]
        latent = rearrange(self.latent,'b w h c-> b c w h') ### (b=1, c=3, w=1024, h=1024)
        latent_img = torch.nn.functional.grid_sample(latent.to(self.device), pts_uv, align_corners=True) ### (b=1, c=3, w=n_pts=300000, h=1)
        latent_img = rearrange(latent_img, 'b c w h -> b (w h) c') ### (b=1, w=n_pts=300000, c=3)->[1, 3, 300000]

        n_pts = latent_img.shape[1]
        n_channels = latent_img.shape[2]
        pulsar = PulsarLayer(n_pts, self.height, self.width, n_channels, 
                             device=self.device).to(self.device)
        pts3d = pts3d.float().to(self.device)
        img_hat = pulsar(pts3d, latent_img, vert_radii, rvec, Cs, Ks)  ##[1, 1024, 1024, 3]     
        img_hat = rearrange(img_hat,'b w h c -> b c w h')##[1, 1024, 1024, 3] -> ]1, 3, 1024, 1024
        ...
        if (batch_idx==view_cam-1):
            ...
            img_path = join(pulsar_dir,"epoch_%03d_cam_%02d.png"%(trainer.current_epoch,view_cam))
...
class ZJUDataModule(pl.LightningDataModule):
    def __init__(self,root_dir:str, person_ids:list,sampling_points:int=300000,batch_size:int=4):
        super().__init__()
    ...
    def setup(self,stage:str=None):
        if stage in (None,"fit"):
            ...
            for person_id in self.person_ids:
                ...
                person_dataset = PersonDatasetPulsar(person,sampling_points=self.n_pts,transform=trans)
    ...
    def train_dataloader(self):
        zju_train = DataLoader(self.data_set,batch_size=self.bs,shuffle=False, num_workers=8,
...
if __name__=="__main__": 
    ...
    try:                
        ...
        my_latentModel = LitUNet(training_path, latent_dim=latent_dim, 
                                 person=person, bs=bs, 
        ...
        trainer.fit(model=my_latentModel, train_dataloaders=zju_trainLoader)  ### for 3 channels NO UNet
