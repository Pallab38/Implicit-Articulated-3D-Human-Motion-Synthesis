...
class LatentCodes:
    def __init__(self, latent_dim: int , training_path: str, subjects: List[str],
                    current_epoch: int, device: torch.device, latent_texture_size : int = 512):
        self.device = device
        ...
        if not isdir(self.latent_img_path):
            assert (current_epoch == -1), f"Cannot resume with empty latent code: {current_epoch}, from {self.latent_img_path}"
    ...
    def save_all_codes(self, latent_image, epoch=-1):
        ...
        for sid in self.data.keys():
            ...
            torch.save({"optim": optim.state_dict()}, fname_optim) ### optimizer dictionary
