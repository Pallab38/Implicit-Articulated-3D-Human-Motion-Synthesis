...
class PersonPulsar:
    ...
    def __init__(
        self, person_id: str, mesh_number: List[int], root="/home/user/my_zjuMocap/"
    ):
...
if __name__ == "__main__":
    ...
    for data in tqdm(person1_dataloader, desc="data"):
        ...
        pts3d, pts_uv, normal_noise = (
            ...
            data["pts_uv"],
            data["normal_noise"],
        )  ## torch.Size([4, 10000, 3]) torch.Size([4, 10000, 2]) torch.Size([4, 10000, 1])
        pid, imgs, masks = (
            data["pid"],
            data["image"],
            data["mask"],
        )  ####  Image : torch.Size([4, 1024, 1024, 3]) Mask:  torch.Size([4, 1024, 1024])
        # print("Image :", imgs.shape,"Mask: ", masks.shape)
        rvec, Cs, Ks = (
            data["rvec"],
            data["Cs"],
            data["Ks"],
        )  ## rvec:  torch.Size([4, 3]) Cs:  torch.Size([4, 3]) Ks:  torch.Size([4, 3, 3])
        vert_radii = data[
            "vert_radii"
        ]  ## v_col:  torch.Size([4, 10000, 3]) v_rad:  torch.Size([4, 10000, 1])
