...
def down(in_ch, out_ch, with_bn=True):
    layers=[
        nn.Conv2d(in_ch, out_ch, kernel_size=3, 
                  padding=1, stride=2, bias = not with_bn),
        nn.ReLU(inplace=True),        
    ]
    if with_bn:
        layers.append(nn.BatchNorm2d(out_ch))
    else: 
        layers.append(nn.GroupNorm(num_groups=8, num_channels=out_ch)) ### num_channels should be divided by num_groups
...
class UNet(nn.Module):
    def __init__(self,in_channels, out_channels, 
        ...
        self.mid2 = middle(8*mid_channels+2*mid_channels, 2*mid_channels, with_bn )##cat[m1,d1]
        
        self.up1 = up(2*mid_channels+ 2* mid_channels, mid_channels, with_bn)##cat[m2,d2]
        self.up2 = up(mid_channels+mid_channels, mid_channels//2) ##cat[u1,d1]
        
        self.out = nn.Conv2d(in_channels=mid_channels//2, out_channels=out_channels,
