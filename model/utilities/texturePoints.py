...
class TexturePoints3D:
    def __init__(self, person_ids:List[str], mesh_num:int, n_sample:int, current_epoch:int,
                 save_path:str, device:torch.device, data_root: str="/home/user/my_zjuMocap") -> None:
        ...
        **Texture Points** are w/o batch.
        ...
        if not isdir(self.save_dir):
            assert (current_epoch== -1), f"Requires Learned Texture Points for {self.current_epoch} at {self.save_dir}"
        ...
        if(self.current_epoch==-1):
            ...
            base_tex3d = torch.FloatTensor(self.n_sample, 3).uniform_() ## torch.float32, [300000, 3])
            ...
            pts3d, pts_uv, normal_noise, noramls = proj.random_sample(n_sample,V) ### (300000, 2) 0.01966 0.9945
            ### Convert Value Range: [0,1]->[-1,1]
            pts_uv = 2.*(pts_uv - np.min(pts_uv))/np.ptp(pts_uv) - 1 ## (300000, 2), -1.0, 1.0)            
            noise_fixed = self.rng.uniform(-0.1, 0.1, (self.n_sample, 1) ) ## (300000, 1)
            uvw = np.concatenate((pts_uv, noise_fixed), axis=1) ## (300000, 3)
            base_tex3d = torch.from_numpy(uvw.copy()).float() ## torch.float32, [300000, 3])
            # print(f"base_tex:{type(base_tex3d), base_tex3d.dtype, base_tex3d.shape}")
            ...
            for pid in self.person_ids:                
                ...
                optim = torch.optim.Adam([tex_pts], lr=0.05, amsgrad=True, weight_decay=0)
            ...
            for pid in self.person_ids:
                ...
                tex_pts.requires_grad = True
                optim = torch.optim.Adam([tex_pts], lr=0.005, 
    ...
    def __init__(self, person_ids:List[str], mesh_num:int, n_sample:int, current_epoch:int,
        ...
        **Texture Points** are w/o batch.
        ...
        if(self.current_epoch==-1):
            ...
            torch.manual_seed(1310)
            self.rng = default_rng(12345)
            base_tex3d = torch.FloatTensor(self.n_sample, 3).uniform_().to(self.device) ## torch.float32, [300000, 3])
            ...
            for pid in self.person_ids:                
                ...
                tex_pts.requires_grad = True
                optim = torch.optim.Adam([tex_pts, tex_2d], lr=0.05, amsgrad=True, weight_decay=0)
...
if __name__ == "__main__":
    ...
    texture_points = TexturePoints3D(person_ids, mesh_num, n_sample, currentEpoch,
                                   save_path, device)
