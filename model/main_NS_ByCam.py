...
class PainterDensityNet(torch.nn.Module):
    ...
    def forward(self, painter_in, density_in, batch):
        out_painter = self.painter(
            painter_in
        )  ## [1, 300000, 32]  -0.2069895714521408, 0.40168941020965576
        out_density = self.density(
            density_in
        )  ## [bs, n_pts, 1] 0.509977400302887, 0.5034168362617493
...
class UnetPainterDensityNet(torch.nn.Module):
    ...
    def __init__(
        ...
        self.painter = SimpleMLP(input_dim=in_painter, net_type="painter")
        self.density = SimpleMLP(input_dim=in_density, net_type="density")
        self.pulsar = PulsarLayer(
            n_pts, height=1024, width=1024, n_channels=in_ch, device=self.device
        ).to(self.device)
        self.neuralShader = ShallowNeuralShader(
            input_channels=in_ch, output_channels=out_ch
        )

    def forward(
        self, painter_in: torch.tensor, density_in: torch.tensor, batch: dict
        ...
        pts_3d, gt_mask = (
            batch["pts3d"].float(),
            batch["mask"],
        )  ## [bs, n_pts, 3], [bs, ch, h, w]
        vert_radii, rvec = batch["vert_radii"], batch["rvec"]  ## [bs, n_pts,1], [bs, 3]
        Cs, Ks = batch["Cs"], batch["Ks"]  ### [bs, 3], [bs, 3, 3]
        pulsar_out = self.pulsar(
            pts_3d, pulsar_in, vert_radii, rvec, Cs, Ks, opacity=out_density
        )
...
class LitPainterDensity(pl.LightningModule):
    def __init__(
        self, my_painterDensity: torch.nn.Module, shallow_neuralSahder: torch.nn.Module
    ):
        super().__init__()
    ...
    def get_input_for_network(self, batch, net_type="painter"):
        ...
        latent = rearrange(
            latent, "b w h c -> b c w h"
        )  ##[bs, w=512, h=512, ch=3]->[1, 3, 512, 512]
        pts_tex = pts_uv.unsqueeze(2)  ## [bs, n_pts, 2]->[bs, n_pts, 1, 2]
        latent_image = torch.nn.functional.grid_sample(
            latent, pts_tex, align_corners=True
        ...
        if net_type == "painter":
            ...
            my_input = torch.cat((gamma_uvw, latent_smpl, fft_normDirection), dim=2).to(
                torch.float32
            )  ## [1, 300000, 1096]
    ...
    def save_all_images(
        ...
        pid: str,
        view_cam: int,
        shader_img: torch.tensor,
        shader_pred_img: torch.tensor,
        pulsar_img: torch.tensor,
    ...
    def training_step(self, batch, batch_idx):
        ...
        painter_input = self.get_input_for_network(
            batch, net_type="painter"
        )  ### [bs,n_pts,1096],
        density_input = self.get_input_for_network(
            batch, net_type="density"
        )  ### [bs,n_pts,584],

        out_painter, out_density = self.painterDensity(
            painter_input, density_input, batch
        )
        ...
        Cs, Ks = batch["Cs"], batch["Ks"]  ### [bs, 3], [bs, 3, 3]
        pulsar = PulsarLayer(
            n_pts, height=1024, width=1024, n_channels=33, device=self.device
        ).to(self.device)
        ...
        pulsar_out = pulsar(
            pts_3d, pulsar_in, vert_radii, rvec, Cs, Ks, opacity=out_dense
        )  ### [bs, h=1024, w=1024, ch=33]
        ...
        pulsar_img = pulsar_out[
            :, 1:4, :, :
        ]  ### [1, 3, 1024, 1024] (-0.6411585807800293, 0.0) w/ density
        ...
        gt_norm = (
            2 * (gt_img - torch.min(gt_img)) / (torch.max(gt_img) - torch.min(gt_img))
            - 1
        )  ### (-1.0, 1.0)
        gt_image = (
            gt_norm * gt_mask
        )  ## [1, 3, 1024, 1024],torch.float32, 0.0 , 0.0039 (-1.0, 1.0)
        l1_loss = nn.L1Loss()
        ...
        pred_mask = (pulsar_mask - torch.min(pulsar_mask)) / (
            torch.max(pulsar_mask) - torch.min(pulsar_mask)
        )  ### [1, 1, 1024, 1024] [0,1] 0.0, 1.0
    ...
    def test_epoch_end(self, outputs):
        ...
        metric_txt = join(
            "/home/user/person_313/NeuralShader/painterDensity/TestMetrics/SummaryMetrics"
        )
    ...
    def test_step(self, batch, batch_idx):
        ...
        pulsar_norm = (pulsar_img[:, :, :, :] + 1) / 2
        pulsar_pred_image = pulsar_norm * gt_mask
        # print(f"pulsar_pred_image: {torch.min(pulsar_pred_image).item(), torch.max(pulsar_pred_image).item()}")
        self.save_pulsar_img(pulsar_pred_image, batch_idx, cid)
        ...
        shader_img, shader_mask = self.neuralShader(pulsar_out)
        ### [-1,1] to [0,1]
        shader_pred_norm = (shader_img[:, :, :, :] + 1) / 2
        # print(f"shader_img: {torch.min(shader_pred_img).item(), torch.max(shader_pred_img).item()}")
        shader_pred_img = shader_pred_norm * gt_mask
        # print(f"shader_img: {torch.min(shader_pred_img).item(), torch.max(shader_pred_img).item()}")
        self.save_cnn_img(shader_pred_img, batch_idx, cid)
        gt_image = gt_img * gt_mask  ## [1, 3, 1024, 1024],torch.float32, 0.0 , 0.0039
    ...
    def configure_optimizers(self):
        ...
        optimizer = optim.Adam(
            [
                {"params": self.painterDensity.parameters()},
...
class ZJUDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str = None):
        ...
        if stage in (None, "fit"):
            ...
            for person_id in self.person_ids:
                person = PersonPulsar(
                    person_id, root=self.root_dir, dataset_type=self.dataset_type
                )
                person_dataset = PersonDatasetPulsar(
                    person,
                    sampling_points=self.n_pts,
                    transform=trans,
            ...
            # print(f"ZJUDATAMODULE: stage type: {stage}")
            # exit()
            for person_id in self.person_ids:
                person = PersonPulsar(
                    person_id,
    ...
    def test_dataloader(self):
        ...
        print(
            "Total : {0}, batch: {1}, batch_size:{2}".format(
                total_frames, len(zju_test), self.bs
            )
        )
        return zju_test
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--run_type", type=str, default="training", help="training or testing"
    )
    parser.add_argument(
        "--cam", type=int, default=5, action="store", help="Camera for test"
    )
    parser.add_argument(
        "--trainedModel", action="store_true", help="Training From Trained Model"
    )
    ...
    parser.add_argument(
        "--resume_training",
        action="store_true",
        help="Resume Training from already trained .ckpt file",
    )
    parser.set_defaults(resume_training=False)

    args = (
        parser.parse_args()
    )  ### Namespace(bs=1, run_type='training', trainedModel=False)
    print(f"arguments from command line: \n{args}")
    run_type = args.run_type
    load_checkpoint = (
        args.trainedModel if run_type == "training" else True
    )  # False #True
    bs = args.bs
    cam = args.cam
    ...
    fh = logging.FileHandler(join(output_dirpath, f"{run_type}NS_{person}_ByCam.txt"))
    ...
    try:
        ...
        latent_dim = 16  ## 16
        enc_dim_x = (
            512  ### After 3D pts/ 3D texture pts encoding with Fourier Feature Mapping
        )
        ...
        model_path = join(
            ...
            "version_5",
            "checkpoints",
            "epoch=199-step=793583.ckpt",
        )  ## Then till 100
        print(f"model_path:{model_path}")
        if load_checkpoint == True or run_type == "testing":
            assert isfile(model_path) == True, "Trained Model Not Found"

        painter_density_net = PainterDensityNet(
            in_painter=in_dim_painter, in_density=in_dim_density
        )
        input_ch, output_ch = 33, 4
        my_neuralShader = ShallowNeuralShader(
        ...
        if args.run_type == "training":
            if args.resume_training:
                ...
                trainer = pl.Trainer(
                    ...
                    callbacks=[MyPrintingCallback(), saveCkptPerEpoch],
                )
            else:
            ...
            print(
                f"Invalid args.run_type= {args.run_type}, accepted arguments 'training' or 'testing' "
            )
